...
class ActiveContract(models.Model):
    ...
    type = fields.Selection(
        [('automatic', 'Auto create rent installment + Invoice'), ('manual', 'List out all rent installment')],
        default='automatic')
    ...
    def action_quarterly_month_active(self):
        ...
        invoice_date = fields.Date.today() + relativedelta(months=3)
        if full_quarter >= 1:
    ...
    def action_yearly_year(self):
        ...
        invoice_date = fields.Date.today() + relativedelta(years=1)
        for i in range(year):
            if i == 0:
                record = {
                    'product_id': self.env.ref('rental_management.property_product_1').id,
